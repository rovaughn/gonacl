...
func Hash(hash []byte, password []byte, opslimit uint64, memlimit uint64) (int, bool) {
	...
	if result := C.crypto_pwhash_str(
		...
		panic("crypto_pwhash_str returned an invalid code")
	}
}

func Verify(hashedPassword, password []byte) bool {
	if result := C.crypto_pwhash_str_verify(
		...
		panic("crypto_pwhash_str_verify returned an invalid code")
	}
}
